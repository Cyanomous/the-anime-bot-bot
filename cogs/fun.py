...
class gifs():
  async def tenor(self, search):
    ...
    async with aiohttp.ClientSession() as cs:
      async with cs.get(f"https://api.tenor.com/v1/search?q={search}&key={tenor_API_key}&limit=50&contentfilter=low") as resp:
  ...
  async def hug(self):
    self.gifs = []
    async with aiohttp.ClientSession() as cs:
      async with cs.get(f"https://api.tenor.com/v1/search?q=animehug&key={tenor_API_key}&limit=50&contentfilter=low") as resp:
        text = await resp.text()
        text = json.loads(text)
        for i in text["results"]:
          for x in i["media"]:
...
class UrbanDictionaryPageSource(menus.ListPageSource):
    ...
    def format_page(self, menu, entry):
        ...
        embed.description = self.cleanup_definition(f"**Definition:**\n {entry['definition']}\n**Example:**\n{entry['example']}")
...
class fun(commands.Cog):
  ...
  def render_emoji(text):
    text_ = text.replace("0", "\U00002b1b").replace("1", "\U00002b1c").replace("2", "\U0001f7e6").replace("3", "\U0001f7eb").replace("4", "\U0001f7e9").replace("5", "\U0001f7e7").replace("6", "\U0001f7ea").replace("7", "\U0001f7e5").replace("8", "\U0001f7e8").replace("9", "")
    return text_
  ...
  async def talk(self, ctx):
    ...
    while talking:
      ...
      try:
        ...
        async with ctx.channel.typing(), aiohttp.ClientSession().post("https://public-api.travitia.xyz/talk", json=payload, headers={"authorization":talk_token}) as res:
  ...
  async def sushi(self, ctx):
      ...
      embed = await embedbase.embed(self, ctx)
      embed.add_field(name="Get the sushi", value=f"**{user.mention} got it in {round(final * 1000)} ms **", inline=False)
