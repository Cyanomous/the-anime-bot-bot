...
class owners(commands.Cog):
  ...
  async def reactionreload(self):
    while True:
        ...
        embed = discord.Embed(color=0x00ff6a, description=f"Reloaded All Cogs \n <:greenTick:596576670815879169> {text}")
  ...
  async def takepic(self, ctx, *, website:str):
    ...
    async with aiohttp.ClientSession(headers={'User-Agent': 'python-requests/2.20.0'}, timeout=aiohttp.ClientTimeout(total=20)).get(website) as resp:
      if resp.status != 200:
    ...
    if ctx.channel.nsfw == False:
      lists = ["dick", "pussy", "horny", "porn", "cum", "cunt", "cock", "penis", "hole", "fuck", "shit", "bitch", "gore", "nsfw"]
    ...
    if any(i in website_ for i in lists2):
      return await ctx.send("can not screenshot that website")
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=20)).get(f"https://image.thum.io/get/png/{website}") as resp:
    ...
    if text_ == "all":
      ...
      for file in os.listdir("./cogs"):
        if file.startswith(f"{text_}.py"):
          self.bot.reload_extension(f"cogs.{file[:-3]}")
          embed = discord.Embed(color=0x00ff6a, description=f" <:greenTick:596576670815879169> Reloaded {file[:-3]}")
          await message.edit(embed=embed)
  @commands.command()
  @commands.is_owner()
