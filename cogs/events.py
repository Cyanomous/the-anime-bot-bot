...
class events(commands.Cog):
  ...
  async def post(self, bot):
      ...
      await session.post("https://top.gg/api/bots/787927476177076234/stats", headers={"Authorization": topgg}, data=json.dumps({"server_count": len(bot.guilds), "shard_count": bot.shard_count}))
      await session.post("https://discordbotlist.com/api/v1/bots/anime-quotepic-bot/stats", headers={"Authorization": discord_bot_list}, data={"voice_connections": len(bot.voice_clients), "users": len(bot.users), "guilds": len(bot.guilds)})
      async with aiohttp.ClientSession().post("https://api.discordextremelist.xyz/v2/bot/787927476177076234/stats", headers={"Authorization": discord_extreme_list, "Content-Type": "application/json"}, data=json.dumps({"guildCount": len(bot.guilds), "shardCount": len(bot.shards)})) as resp:
        pass
      await session.post("https://botsfordiscord.com/api/bot/787927476177076234", headers={"Content-Type": "application/json", "Authorization": bots_for_discord}, data=json.dumps({"server_count": len(bot.guilds)}))
      await session.post("https://api.botlist.space/v1/bots/787927476177076234", headers={"Authorization": botlist_space, "Content-Type": "application/json"}, data=json.dumps({"shards": [len(bot.guilds)/2, len(bot.guilds)/2]}))
      await session.close()
  ...
  async def on_command_error(self, ctx, error):
    ...
    if isinstance(error, commands.DisabledCommand):
      ...
      embed = await self.embed(f"You have hit the global ratelimit try again in {round(error.retry_after)} seconds")
      return await ctx.send(embed=embed)
