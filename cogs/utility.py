...
class utility(commands.Cog):
    ...
    """
        async with self.bot.session.get("https://api.ksoft.si/kumo/currency", headers = json.dumps({"Authorization": os.getenv("ksoft")}), params = {"from": from_, "to": to, "value": amount}) as resp:
            res = await resp.json()
            print(res)
            if res.get("message"):
